...
Private Sub ChargerPlanningSemaine(nomComplet As String)
    ...
    If lrP > 0 Then
        For j = 1 To 7
            ...
            Dim vE As String: vE = CStr(wsP.Cells(lrP, colE).Value)
            Dim vS As String: vS = CStr(wsP.Cells(lrP, colS).Value)
        ...
        For k = 2 To lastC
            If wsC.Cells(k, 1).Value = nomComplet And IsDate(wsC.Cells(k, 2).Value) Then
                If CLng(CDate(wsC.Cells(k, 2).Value)) = CLng(dJour) Then
                    Me.Controls("txtPauseD" & j).Text = CStr(wsC.Cells(k, 5).Value)
                    Me.Controls("txtPauseF" & j).Text = CStr(wsC.Cells(k, 6).Value)
